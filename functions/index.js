...
paypal.configure({
  ...
  client_id: functions.config().paypal.client_id, // run: firebase functions:config:set paypal.client_id="AYHZFMlU5tXJ_c4vnbsOCs4TrftTN-nrjWJ7r_bT3CKOOAvauE6wKTPb8h-KTOu0LbWcNkx-_GHG2tGG"
  client_secret: functions.config().paypal.client_secret // run: firebase functions:config:set paypal.client_secret="ENRBtJCaUoFV6dfiSXpb1OhSsNciiX5V6ptHsY1MqtAa5TDeUbVc0xkDHU8zU_KMUHhBLk9zEIibvUAv"
});
...
exports.pay = functions.https.onRequest((req, res) => {
  ...
  paypal.payment.create(payReq, (error, payment) => {
    ...
    if (error) {
      console.error(error);
      console.log(payReq.redirect_urls)
      res.status('500').end();
    } else {
